...
| Gap | Legacy | Why not ported | Re-entry criterion |
|---|---|---|---|
| **The four empty banks** — `claude-code::user`, `claude-code::bank-f`, `claude-code::bank e`, `codex`, and `claude-code::memgarden`, which appeared during Phase D | five `banks` rows with a mission and disposition and zero content | Nothing to lose — 0 nodes, 0 documents, 0 links — and `hook session-start`'s `POST /v1/banks` (`session_start.rs:159-166`) recreates any of them on first use. Creating them would put a number in the AC-3 report that overstates what was verified. `banks.json` in every snapshot preserves each mission verbatim, including `codex`'s hand-written 149-character one, so the string survives the bank not doing so | A user who wants a bank pre-created before its first prompt, or a bank whose mission was hand-tuned away from the default (none is) |
