...
| ~~**Background mental-model refresh**~~ **CLOSED 2026-08-29** | `maintenance.py:417-425` and the op queue behind it | **The re-entry criterion fired.** The row said "any caller that reads mental models", and `/v1/banks/{id}/reflect` is one — it pulls the bank's nearest models by KNN on every call, so a stale document became a worse answer rather than an unread one. `mental::cron::run_task` now ticks on `[mental] refresh_interval_secs` (default 600, `0` disables), refreshing only models whose own cron has fired since `last_refreshed_at`. The interval bounds *lateness*, not GPU spend: a tick with nothing due costs one `list` per bank, and `refresh` short-circuits with "no new supporting facts, preserving content" before any LLM call — both observed live on the first tick after deploy. Guards borrowed from `consolidate::round::run_task` rather than invented: `0` disables, `MissedTickBehavior::Delay`, and the tick defers entirely while a retain is in flight | **Not a gap.** Reopen only if scheduled refresh is found to spend GPU when nothing is due, which the short-circuit is there to prevent |
