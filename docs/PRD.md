...
### v1 이후 대기 중인 측정
- **정제(distillation) 가치 측정 (미착수) — 이 프로젝트의 최대 미측정 주장.**
  CE-9 컨솔리데이션은 매일 돌아 **관측 2,770건(출처 연결 2,757건)** 을 만들었는데,
  관측이 자기 구성 사실들보다 나은지 **한 번도 비교된 적이 없다.** 골드 하네스로
  `world`만 회수한 팔과 `observation`을 허용한 팔을 비교하면 recall@k·MRR·nDCG로 답한다.
