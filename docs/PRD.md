...
- **배포**: `memgardend`(데몬, REST :포트 configurable) + 웹UI 서버 분리, 독립 재시작 (R2)
...
- [ ] **AC-2 성능**: recall p50 ≤35ms·p95 ≤60ms(로컬), 훅 총 오버헤드 <10ms, retain 캡 절감률(-55~87%) 유지
- [ ] **AC-3 무손실 마이그레이션**: 기존 3개 뱅크 노드·링크·문서 카운트 일치 + 무작위 샘플 50건 내용 대조 통과(검증 스크립트)
- [ ] AC-4 그래프 뷰어: 현재 규모(≥2,500노드) 팬/줌/드래그/호버 원활, retain 반영 ≤5s, 세션·뱅크·기억타입 필터
- [ ] AC-5 대시보드: memdash 전 기능 + HEALTHY/DEGRADED/UNHEALTHY 판정, 10s 자동 갱신
- [ ] AC-6 지표: 이득 원장·Layer1/2 자동 수집, 수집 경로 지연 증가 0 실측 증명
