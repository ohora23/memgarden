...
### v1 이후 대기 중인 측정
- **~~정제 합성기가 철회된 사실을 구분 못 한다~~ 저장은 해결, 검출은 미해결 (CE-12, 2026-08-30).**
  CE-10 복구 후 만든 멘탈 모델 4개 중 1개가 **확신에 차서 틀렸다** — "AC-1은 서명 대기 중",
  "6승/2무/5패"(폐기된 첫 측정), "AC-7 정보 없음". 셋 다 쓰일 당시엔 참이었고 이후 대체됐는데,
  뱅크는 두 버전을 모두 갖고 있었다.
  **해결된 쪽:** 스키마 v11이 `memory_nodes.superseded_by`와 `expires_at`을 추가하고,
  `search::hydrate` **한 곳**에서 거른다 — BM25·KNN·시간·그래프 arm과 `/reflect`,
  멘탈 모델 리프레시가 전부 이 지점을 지나므로 `mental/`은 한 줄도 고치지 않았다.
  설정 수단은 `POST`/`DELETE /v1/banks/{bank}/nodes/{id}/supersede` 두 동사다.
  **미해결인 쪽:** 자동 검출. 이 프로젝트가 실제로 기록한 철회 4건 + 대조 3건으로 재보니
  1차형은 7청크에 **오탐 22건**(한 청크는 후보 12개 전부를 철회한다고 답했다), 오탐 0인
  형태는 **검출도 0**이었다. 세 팔 전부와, 필드를 `required`로 바꿨을 때 생긴 문법 폭주·잘림까지
  `docs/evidence/supersession-detection.md`. **기본값 `detect_supersession = false`**,
  끈 상태의 추출 프롬프트는 CE-12 이전과 **바이트 단위로 동일**하다(6,999 / 7,615자 스냅샷).
  프롬프트로 최신 우선을 지시하는 건 여전히 두 번 틀렸다 — 최신이 항상 옳지 않고, 14B가
  프롬프트 지시를 무시하는 건 이번에도 다시 관측됐다(arm B).
  다음에 열어 볼 팔은 **비용 때문에 건너뛴 두 번째 LLM 호출** 하나다.
  `docs/evidence/mental-model-supersession.md`
- **~~`trigger`를 API로 끌 수 없다~~ 해결 (2026-08-30).**
  `DELETE /v1/banks/{bank}/mental-models/{mm_id}/trigger`. 필드가 아니라 **동사**다 —
  `PATCH`의 `Option<String>`에서 `None`은 "건드리지 마라"라 JSON `null`과 키 생략이
  구분되지 않고, 그건 그대로 뒀다(바디의 `null`이 여전히 아무 뜻도 아님을 테스트가 못 박는다).
  스토어 쪽은 `mental_models::Patch::clear_trigger`로, 같은 이유로 이미 있던
  `clear_embedding` 플래그와 같은 모양이다. 모델이 없거나 트리거가 없으면 둘 다 404 —
  "여기 끌 스케줄이 없다"로 뜻이 같고 운영자의 다음 동작도 같다.
  CE-12의 `superseded_by`도 같은 방식으로 냈다(`POST`로 걸고 `DELETE`로 푼다).

- **`<private>` 옵트아웃 (2026-08-30, claude-mem에서 도입).** 대화 중 `<private>…</private>`로
  감싼 텍스트는 추출이 보기 전에 잘려나간다. **닫히지 않은 마커는 그 메시지의 나머지를 버린다** —
  바로 옆 `remove_blocks`(메모리 주입 래퍼용)와 실패 방향이 반대인데, 대상이 산출물이 아니라
  비밀일 땐 보수적인 쪽이 반대이기 때문이다. 한계도 같이 적는다: 메시지 텍스트만(도구 페이로드
  제외), 소문자 정확 일치, retain 시점 한정(이미 저장된 건 못 지운다).
- **~~정제(distillation) 가치 측정~~ 완료 (2026-08-29) — 값을 한다.** 골드 코퍼스 3팔,
  변수는 `recall_types` 하나: **recall@10 0.291 → 0.370(+27%)**, **nDCG@10 0.262 → 0.306(+17%)**.
  **정답 라벨이 71% `world`인 결에 거슬러 이긴 것**이라 신호가 강하다.
  다만 **관측 단독은 진다**(recall@10 0.177) — 정제는 사실을 *대체*하지 않고 *보완*한다.
  **유일한 손해가 `conclusion` 계층**(nDCG −0.018, MRR 0.200→0.143)이고, 하필 정제가 가장
  값을 해야 할 곳이다. 채점 질의 1건이라 발견이 아니라 경고다.
  `docs/evidence/distillation-value-result.md`
- **~~CE-10 멘탈 모델 — 한 번도 안 돎(0행)~~ 수리 완료 (#39–#42, 2026-08-29/30).**
  멘탈 모델 4개가 존재하고 3개가 `@after-consolidation`으로 돈다. 안 돌던 진짜 이유는
  호출자 부재 **하나가 아니었다** — 모든 호출이 Ollama 문법 한계로 실패하는 **100% 죽은 코드**였고,
  호출자가 없어서 두 달간 아무도 몰랐다. **남은 것은 승격/강등 규칙이고 데이터가 막고 있다**:
  `/reflect`가 어느 훅 경로에도 없어 `cited_count`가 전부 0이다. 신호에 값이 생기기 전에 규칙을
  쓰는 건 CE-10을 두 달간 고장 낸 것과 정확히 같은 모양이다.
  아래는 수리 이전의 기록이다.
  due 판정(`mental/cron.rs`)과 생성 경로
  (`routes/mental.rs`)는 있는데 호출자가 없다. `parity-gaps.md`가 *"소비자 없이 GPU를 쓰는
  스케줄러는 잘못된 기본값"* 이라 판단했고 당시엔 타당했다. 다만 **사용자가 이 시스템을 만든
  세 이유 중 "주기적 정제·고도화"가 정확히 이것**이라, 티커와 소비자(회수가 멘탈 모델을 주입)를
  같이 붙여야 의미가 생긴다.
  **2026-08-29 갱신 — 켤 근거는 생겼고 조건이 붙는다.** 아래 정제 측정이 소비자 쪽 근거를 줬지만
  두 제약을 같이 정했다: ① 관측 단독이 지므로 멘탈 모델도 **대체가 아니라 추가 진입점**으로,
  ② `conclusion`이 유일한 손해였으므로 **결론형 질의로 먼저 재고** 켤 것.
- **⚠️ 코퍼스 센서스(0/60)를 정제 가치의 근거로 인용하지 말 것.** 특징 용어가 디스크에 있는지
  보는 검사라 **합성물을 판정할 수 없다** — 40개 세션을 압축한 관측은 구성 사실이 전부 디스크에
  있어도 그 형태는 없다. 원래 질문(원사실 유일성)에는 유효하고 정제에는 무효다.
  `docs/evidence/bank-uniqueness-result.md`
