...
- [ ] AC-7 전 PR이 템플릿 준수(PRD ID + 검증 증거), `cargo test` 통과
  ...
  — **`cargo test` 조항은 미결.** 커널 `-29`에서 4회 중 2회 SIGSEGV, `-30`에서 55회 중 0회. 원인은 이 코드베이스가
  아니라 커널 폴트다(`/var/crash/` 덤프 3건, 전건 CPU 3, 그중 하나는 유휴 태스크 `swapper/3`). uptime 1.7·13·31시간에
  발생한 사건이라 갓 부팅한 상태의 무결 55회는 근거가 못 된다. `-30`이 31시간 uptime을 조용히 넘기면 닫힌다.
