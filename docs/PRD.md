...
### v1 이후 대기 중인 측정
- **CPU-3 대조 실험 (미착수).** treatment(코어 오프라인, 22h43m 무사고)는 기록됐다. control은
  `echo 1`로 `cpu3`/`cpu11`을 올리고 같은 커널·같은 부하로 12시간 소크. 조용하면 `-29` 커널 버그 확정,
  패닉하면 새 덤프의 CPU 번호가 답한다. 재부팅하면 오프라인이 풀려 control이 저절로 시작된다.
...
- **명령 로그 178건 정리 (미착수).** 남은 문제는 유입이 아니라 **이미 쌓인 178건이 자기 몫의 9배로
  회수되는 것**이다(저장 2.4% 대 주입 22%). 전량 마이그레이션·캐치업 산물. 분류기 정밀도 92%라
  오탐 검토와 DB 백업이 선행 조건. `docs/evidence/command-log-pollution.md`
