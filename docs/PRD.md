...
### v1 이후 대기 중인 측정
- **정제 합성기가 철회된 사실을 구분 못 한다 (미해결, 2026-08-30 관측).** CE-10 복구 후 만든
  멘탈 모델 4개 중 1개가 **확신에 차서 틀렸다** — "AC-1은 서명 대기 중", "6승/2무/5패"(폐기된
  첫 측정), "AC-7 정보 없음". 셋 다 쓰일 당시엔 참이었고 이후 대체됐는데, 뱅크는 두 버전을
  모두 갖고 있다. **`created_at`은 있지만 "B가 A를 대체한다"를 기록하는 곳이 없다.**
  철회가 일급 사건인 이 프로젝트에서 특히 아프다. 프롬프트로 최신 우선을 지시하는 건 두 번
  틀렸다 — 최신이 항상 옳지 않고(정정도 정정된다), 14B가 프롬프트 지시를 무시하는 건 이미
  관측됐다. **정제층은 틀림이 복리로 커지는 유일한 곳**이다: 낡은 원사실은 7,683개 중 하나지만
  낡은 합성물은 권위 있게 읽히고, 인용되고, 주기적으로 갱신된다.
  후보 셋 다 비용이 있고 측정이 선행이다. `docs/evidence/mental-model-supersession.md`
- **`trigger`를 API로 끌 수 없다 (미해결).** `PATCH`의 `Option<String>`에서 `None`은
  "건드리지 마라"라 JSON `null`과 키 생략이 구분되지 않는다. 트리거 없이 **만들 수는** 있는데
  **끌 수는** 없다 — 모델이 이상한 걸 뱉기 시작했을 때 운영자가 가장 먼저 찾는 동작이고,
  실제로 위 사례에서 SQL로 껐다.
...
- **CE-10 멘탈 모델 — 한 번도 안 돎(0행).** due 판정(`mental/cron.rs`)과 생성 경로
