...
recurrence markers (`every|each <time unit>`, `daily/weekly/…`,
`매일/매주/매달/격주/…`) and an open-ended `before <period>` / `<period> 이전`,
which has no lower bound. `since <period>` / `<period>부터` is the opposite
case and *is* a range, `[period start, now]` — legacy `since_constraint`.
...
ordering behaviour: `지난주말` *contains* `지난주`, so the weekend rules run
before the week rules — the reverse of legacy's order, which is safe only
because `\blast week\b` does not match inside "last weekend".
...
* **`this`/`next` periods and the whole Korean column are additions.**
  Legacy's non-Chinese set is past-only; its *Chinese* set has 这周/下周, so
  the concept is legacy-supported, just not in the Latin-script module.
...
* `이번 주말` has no rule of its own, so it matches `이번 주` and answers with
  the whole week — a superset, not a wrong answer.
* The arm's `coalesce()` defeats `idx_memory_nodes_occurred`, so it is a
  bank-partition scan. Measured cost below; the upgrade path (a stored
  `effective_at` generated column with its own index) is in the ponytail
  comment on the query.

## Verification

`cargo test --workspace`: **295 passed, 0 failed, 6 ignored** — up from 273 at
...
sets; the weekend-before-week ordering; range end at the last millisecond of
its last day; `Unconstrainable` short-circuiting *before* both the period rules
and the fallback, with the same queries minus the recurrence marker proving the
assertion is not vacuous; `every` needing a time unit; before-vs-since;
case-insensitive matching with the original reaching the fallback; the arm's
...
dateless node, and the zero-width window; `temporal_boost` at 0.0/0.5/1.0;
end-to-end, the arm pulling in a node BM25 cannot reach and that node
disappearing when the temporal expression leaves the query; the same in Korean.

### Measured — temporal arm latency

`cargo test --release -p memgardend --test recall_api -- --ignored --nocapture
temporal_arm_bench`, 3000 nodes, half carrying `occurred_start` and half only
`mentioned_at` so both COALESCE branches are exercised, 200 samples:
...
The bench now spreads `mentioned_at` over the last 90 days and rotates **seven**
queries, two of them temporal (one Korean) — so the fourth arm is live on ~29%
of requests instead of returning empty. Real `bge-small-en-v1.5`, 3000 nodes,
2000 requests.
...
**AC-2 (p50 ≤ 35 ms, p95 ≤ 60 ms) holds in all three runs.** Idle is flat
against CE-7 (+0.09 ms p95) — the arm costs what its SQL costs and no hop.

**The loaded trend continues and is worth stating plainly.** Loaded p95 headroom
to the 60 ms gate: 18.1 ms at CE-6 → 11.3 ms at CE-7 → **5.1 ms (worse run) /
7.5 ms** here. p99 is now *above* the gate (63–65 ms) and the fraction under
60 ms slipped from 1997/2000 to 1941–1957/2000. Two runs are reported because
this number moves several ms between runs: the loaded case is dominated by the
background ingest's write-lock and connection-pool pressure and by the vec0
partition growing under the recall loop (36,560 / 36,648 extra nodes written
during these runs, vs 35,840 at CE-7).

**The known lever was not taken.** CE-7 recorded merging the graph arm's two
hops (expand + hydrate into the main hop) as the reserve lever. It is still
available and still unspent: CE-8 was implemented to *avoid* needing it — the
temporal arm folds into the existing hop instead of adding a fourth — and p95
still passes with margin in both runs. Taking it now would mean moving the
pass-1 filtering and fusion inside a blocking closure, a real refactor of the
recall core, to buy headroom on a gate that is not failing. The trigger to
spend it: a loaded p95 above 55 ms, or a p50 that starts climbing.
